...
module.exports = deploymentWithProposal(
  { deployName: "094_deploy_aave_dai", forceDeploy: true, tags: ["test", "main", "aave_dai"], dependencies: [] },
  async ({
    oracleAddresses,
    assetAddresses,
    ...
    console.log("4. Init and configure new AaveSupplyStrategy for DAI")
    const initFunction =
      "initialize(address,address,address[],address[],address[],address,address[])";
    await withConfirmation(
      cAaveSupplyStrategy.connect(sDeployer)[initFunction](
        assetAddresses.aDAI, // platform address(aave aToken address)
        cVaultProxy.address, // vault address
        [assetAddresses.USDC], // reward token (USDC)
        [assetAddresses.DAI], // assets (USDC)
        [assetAddresses.aDAI], // pToken address (aToken)
        assetAddresses.USDC, // primary token
        [assetAddresses.aaveLendingPool, // Lending pool
        assetAddresses.aDAI], // aToken
        await getTxOpts()
      )
    );
