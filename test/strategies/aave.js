...
describe("Aave Strategy", function () {
  ...
  describe("Mint", function () {
    it("Should be able to mint DAI and it should show up in the Aave core", async function () {
      ...
      await justMint("30000.00", dai);
      await expectApproxSupply(cash, cashUnits("30200"));
      // should allocate all of it to strategy
      await expect(aaveStrategy).has.an.approxBalanceOf("30200", adai);
      await expect(anna).to.have.a.balanceOf("30000", cash);
    ...
    it("Should not send USDC to Aave strategy", async function () {
      await emptyVault();
      // should be all empty
      await expectApproxSupply(cash, cashUnits("0"));
      await justMint("30000.00", usdc);
      await expectApproxSupply(cash, cashUnits("30000"));
      await expect(aaveStrategy).has.an.approxBalanceOf("0", dai);
      await vault.connect(anna).redeem(cashUnits("30000.00"), 0);
    });

    it("Should be able to mint and redeem DAI", async function () {
      await expectApproxSupply(cash, cashUnits("200"));
      await justMint("30000.00", dai);
      await vault.connect(anna).redeem(cashUnits("20000"), 0);
      await expectApproxSupply(cash, cashUnits("10200"));
      // Anna started with 1000 DAI
      await expect(anna).to.have.a.balanceOf("21000", dai);
      await expect(anna).to.have.a.balanceOf("10000", cash);
    });
    ...
    it("Should be able to redeem and return assets after multiple mints", async function () {
      await justMint("30000.00", usdt);
      await justMint("30000.00", usdc);
      await justMint("30000.00", dai);
      await vault.connect(anna).redeem(cashUnits("60000.00"), 0);
    ...
    it("Should allow transfer of arbitrary token by Governor", async () => {
      ...
      await vault.connect(anna).justMint(dai.address, daiUnits("8.0"), 0);
      // Anna sends her CASH directly to Strategy
      await cash.connect(anna).transfer(aaveStrategy.address, cashUnits("8.0"));
      // Anna asks Governor for help
      await aaveStrategy
        .connect(governor)
        .transferToken(cash.address, cashUnits("8.0"));
    ...
    it("Should not allow transfer of arbitrary token by non-Governor", async () => {
      // Naughty Anna
      await expect(
        aaveStrategy.connect(anna).transferToken(cash.address, cashUnits("8.0"))
  ...
  describe("Rewards", function () {
    ...
    const collectRewards = function (setupOpts, verificationOpts) {
      return async function () {
        ...
        await aave.connect(governor).justMint(stkAaveAmount);
        ...
        if (stkAaveAmount > 0) {
          await stkAave.connect(governor).justMint(stkAaveAmount);
          await stkAave
            .connect(governor)
