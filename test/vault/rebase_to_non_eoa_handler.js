...
describe("RebaseToNonEoaHandler", function () {
  it("Should be able to add contracts @fork", async () => {
    const { vault, governor, rebaseToNonEoaHandler, rio, anna, john } = await loadFixture(
      ...
      await rebaseToNonEoaHandler
        .connect(governor)
        .addContract(vault.address, samplePoolId, rio.address, anna.address, 800000, 150000, 50000, false);
  ...
  it("Simulate the non-rebase to rebase transfer  @mock", async () => {
    ...
    await rebaseToNonEoaHandler
      .connect(governor)
      .addContract(uniswapV2PairCASHUSDC.address, samplePoolId, cDystopiaStrategyDaiUsdt.address, cDystopiaStrategyUsdcDai.address, 800000, 150000, 50000, true);
  ...
  it("Should not be able to double add contracts @fork", async () => {
    const { vault, governor, rebaseToNonEoaHandler, rio, anna, john } = await loadFixture(
      ...
      await expect(  rebaseToNonEoaHandler
        .connect(governor)
        .addContract(vault.address, samplePoolId, rio.address, anna.address, 800000, 150000, 50000, false)).to.be.revertedWith('ALREADY_ADDED');
      await rebaseToNonEoaHandler.setTreasury(john.address);
