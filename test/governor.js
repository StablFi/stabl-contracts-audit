...
describe("Can claim governance with Governor contract and govern", () => {
  it("Can claim governance and call governance methods @mock", async () => {
  ...
  it("Can claim governance and call governance methods in multiple calls @mock", async () => {
    const fixture = await loadFixture(defaultFixture);
  ...
  it("Should not allow anyone else to propose and queue @mock", async () => {
    const fixture = await loadFixture(defaultFixture);
  ...
  it("Should be able to do one call to rule them all[Push simulation here] @mock", async () => {
    const fixture = await loadFixture(defaultFixture);
    const { cMeshSwapStrategyUSDC, vault, governor, governorContract, cash } =
      fixture;

    // Transfer everyone to the governorContract
    await vault.connect(governor).transferGovernance(governorContract.address);
    await cash.connect(governor).transferGovernance(governorContract.address);
    await cMeshSwapStrategyUSDC
      .connect(governor)
    ...
    expect(
      await (await ethers.getContractAt("Governable", cash.address)).governor()
    ).to.be.equal(governorContract.address);
    expect(
      await (
        await ethers.getContractAt("Governable", cMeshSwapStrategyUSDC.address)
      ).governor()
    ).to.be.equal(governorContract.address);
    expect(await vault.totalValue()).to.be.equal(54);
  });

  it("Should be able transfer governance @mock", async () => {
    const fixture = await loadFixture(defaultFixture);
    const { governorContract, vault, governor } = fixture;
  ...
  it("Can cancel queued and pending transactions @mock", async () => {
    const fixture = await loadFixture(defaultFixture);
    const { vault, governor, governorContract } = fixture;
    //transfer governance
    await vault.connect(governor).transferGovernance(governorContract.address);
  ...
  it("Should not allow cancelled events to be queued/executed @mock", async () => {
    const fixture = await loadFixture(defaultFixture);
    const { vault, governor, governorContract } = fixture;
  ...
  it("Should not allow executing a proposal with a setPendingAdmin transaction @mock", async () => {
    const fixture = await loadFixture(defaultFixture);
    const { vault, governor, governorContract, anna } = fixture;
  ...
  it("Should allow admin to call setPendingAdmin @mock", async () => {
    const fixture = await loadFixture(defaultFixture);
