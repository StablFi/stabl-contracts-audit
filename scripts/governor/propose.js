...
// Transfer governance of the CASH contract from old to new governor.
// IMPORTANT: must be executed against the old governor.
async function proposeCASHNewGovernorArgs() {
  ...
  const description = "CASH governance transfer";
  const args = await proposeArgs([
    {
      contract: cCASH,
...
// - upgradeTo CASH
async function proposeCASHv2ResetArgs() {
  const dCASH = await ethers.getContract("CASH");
  const dCASHReset = await ethers.getContract("CASHReset");
  const cCASHProxy = await ethers.getContract("CASHProxy");
  const cCASHReset = await ethers.getContractAt(
    "CASHReset",
    cCASHProxy.address
  );
  const cVaultProxy = await ethers.getContract("VaultProxy");

  const description = "CASH Reset";
  const args = await proposeArgs([
    {
      contract: cCASHProxy,
      signature: "claimGovernance()",
...
// Returns the argument to use for sending a proposal to upgrade CASH.
...
// Returns the argument to use for sending a proposal to upgrade CASH.
async function proposeUpgradeCASHArgs() {
  ...
  const description = "Upgrade CASH";
  return { args, description };
}
...
async function main(config) {
  ...
  } else if (config.upgradeCASH) {
    console.log("upgradeCASH proposal");
    argsMethod = proposeUpgradeCASHArgs;
  ...
  } else if (config.cashNewGovernor) {
    console.log("CASHNewGovernor");
    argsMethod = proposeCASHNewGovernorArgs;
  } else if (config.cashv2Reset) {
    console.log("CASHv2Reset");
    argsMethod = proposeCASHv2ResetArgs;
...
const config = {
  ...
  cashNewGovernor: args["--cashNewGovernor"],
  cashv2Reset: args["--cashv2Reset"],
